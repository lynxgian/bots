...
module.exports = {
  name: "warn", //Command name, same as file name.
  description: "Warn a member", //Description
  ...
  run: async (bot, message, args) => {
    ...
    let warns = memberinfo.warns+1;
    let reason = args.slice(1).join(" ")
    let delivered = false;
    try{
      await target.send(`:no_entry_sign: **| You have been warned by ${message.author}${reason?` for: '${reason}'`:""}. Actual warnings: ${warns}****`);
      delivered = true;
    }catch(e){
      delivered = false
    }
    bot.database.prepare(`UPDATE membersinfo SET warns=? WHERE userid=?`).run(warns,target.id)
    message.channel.send(`${bot.emoji.check} **| You have warned ${target.user.tag}${reason?` for: '${reason}'`:""}. Actual warnings: ${warns}**`).then(m=>{
